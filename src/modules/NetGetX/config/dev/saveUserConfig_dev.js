...
const CONFIG_DIR = path.join(os.homedir(), '.get/dev');
const USER_CONFIG_FILE = path.join(CONFIG_DIR, 'userConfigX_dev.json');
 ...
 */
async function saveUserConfig_dev(updates) {
    try {
        ...
        console.log(chalk.green('userConfigX_dev Updated successfully.'));
    } catch (error) {
...
export default saveUserConfig_dev;
